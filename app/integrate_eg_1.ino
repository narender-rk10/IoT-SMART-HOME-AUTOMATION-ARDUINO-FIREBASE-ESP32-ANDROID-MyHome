...
String formattedDate, formattedTime, dayStamp,timeStamp,autoday,withoutsec,months, finalMonth, niid;
int finalCurrent, finalPower, finalEnergy, monthint, monthdiff;
...
if(p4oftime.equals(withoutsec))
{
  ...
  if( !isnan(current) ){
        ...
        {
          int oldVal=firebaseData.intData();
    ...
    if( !isnan(power) ){
     ...
     if( !isnan(energy) ){
        if(Firebase.getFloat(firebaseData, "users/3Oou95cKpPX6OPBduFNTOHx6sO32/electricity/energy/"+finalMonth+"/"+dayStamp))
        {
          int oldVal=firebaseData.intData();
          float newVal=oldVal+(energy/1000.0f);
          Firebase.setFloat(firebaseData, "users/3Oou95cKpPX6OPBduFNTOHx6sO32/electricity/energy/"+finalMonth+"/"+dayStamp,newVal);
        }
        else
        {
          float newVal=energy/10.0f;
          Firebase.setFloat(firebaseData, "users/3Oou95cKpPX6OPBduFNTOHx6sO32/electricity/energy/"+finalMonth+"/"+dayStamp,newVal);
        }
    }
